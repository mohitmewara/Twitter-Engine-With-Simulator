defmodule Twitter.Server.Memory do
    ...
    def register_new_user(username, password, node_name) do        
        if check_username(username) == false do
            :ets.insert_new(:user_lookup, {username, password, node_name, false, :null, [], [], []})
    ...
    def get_online_status(username) do
        case :ets.lookup(:user_lookup, username) do
            [{_, _, _, status, _, _, _, _}] -> {:ok, status}
    ...
    def authenticate_user(username, password, session_Id) do
        case :ets.lookup(:user_lookup, username) do
            [{username, pass, node_name, login, id, tweet_list, following_list, follower_list}] -> 
                if login == false do
                    if pass = password do
                        :ets.insert(:user_lookup, {username, password, node_name, true, session_Id, tweet_list, following_list, follower_list})
    ...
    def get_hashtag(hashtag) do
        case :ets.lookup(:hashtag_table, hashtag) do
            [{hashtag, tweet_list}] -> {:ok, tweet_list}
            [] -> {:error, "Cannot find the hashtag #{hashtag}"}
        end
    end
    
    def get_username(username) do
        case :ets.lookup(:username_table, username) do
            [{username, tweet_list}] -> {:ok, tweet_list}
            [] -> {:error, "Cannot find the Username #{username}"}
        end
    end    
end