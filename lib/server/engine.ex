defmodule Twitter.Server.Engine do
    ...
    def handle_call({:user_register, username, password, node_name}, _from, state) do        
       {:reply, Memory.register_new_user(username, password, node_name), state}   
    end
    ...
    def handle_call({:follow, followed_username, follower_username, follower_session_key}, _from, state) do
        ...
        case Memory.get_online_status(username) do
            {:ok, login} -> 
                if login == true do
                    ...
                    usernames=  Regex.scan(~r/\B@user[á-úÁ-Úä-üÄ-Üa-zA-Z0-9@._]+/, tweet)
                                            ...
                                            {:ok, status} -> 
                                                if status == true do
                                                    case Memory.get_node_name(follower) do                                                      
                                                       {:ok, node_name} ->                                                        
                                                        GenServer.cast({follower, node_name}, {:receive_tweet, username, tweet}) 
    ...
    def handle_cast({:retweet, username1, username2, session_key, tweet}, state) do
        counter = Enum.at(state, 0)
